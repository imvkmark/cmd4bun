...
- AI 建议不能直接自动落地：提交说明需要用户可见，避免模型误判导致提交说明偏离真实变更。
- 优先基于已暂存 diff 生成说明：当存在 staged 内容时，提交意图通常以暂存区为准；没有 staged diff 时才使用未暂存 diff。
- `--auto` 模式必须先 stage 所有变更：进入自动模式后立即执行 `git add -A`，保证 `git diff --cached` 能拿到完整 diff，避免纯 untracked 场景下 commit message 为空。非 auto 模式保持原流程，由用户在 Accept 路径中通过 `selectFiles` 选择要提交的文件。
