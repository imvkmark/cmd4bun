...
- **Phase 2 不删 aimDirectory 子树**：`sync` Phase 2 清理过期 `.md` 文件时，跳过所有 `feishu.{group}.aimDirectory`（含 `default` fallback）子树下的文件——这些文件由 `copy-docs` 阶段写入，归另一条数据流（归档目录）管辖，`sync` 不干预。即使 `outputDir` 与 `aimDirectory` 实际物理重叠（如 `feishu.dir=./docs`、`default.aimDirectory=./docs/blog`），aimDirectory 子树始终排除。`collectAllAimDirectories` helper 收集所有 group aimDirectory 绝对路径（去重），`classifyLocalFiles` 纯函数按"是否在 aimDirectory 子树内"和"是否在 DB 索引中"做三分类决策（删除 / 排除 / 保留）。日志分两段输出：`删除: N` + `aimDirectory 排除: M`，M > 0 时打印每个排除文件相对路径。详见 [sync 排除 aimDirectory & callout 内部 `<a>` 链接替换](../.req-discuss/feishu/sync-aim-dir-and-callout-link/discuss.md)。
...
- 下载管线解析 `<callout>` 块为 VitePress `:::` container（外壳 emoji→type 映射沿用 `resolveCalloutBlocks`），**块内 `<a href="...">text</a>` 链接做 href 替换但保持 `<a>` 标签形态**（不转 Markdown）：href 为 `http(s)://` 完整 URL 原样保留；其他走与 cite / sub-page 共享的 `makeResolveLink` 闭包——同组 `<a href="${human_path}.md">`、跨组 + aimUrl 可解析 `<a href="${aimUrl}/${human_path}.html">`、跨组 aimUrl 缺失 / 未就绪保留原文 + warning。`makeResolveLink(lookup)` 抽到 `processDocContent` 内部 helper，cite / sub-page / callout 三个解析器共用同一份决策逻辑（差异仅在 lookup 函数：cite 用 `getNode`、sub-page 与 callout 用 `getNodeByObjToken`）。callout 解析器返回的 `warnings` 同样累计到 `unresolvedRefCount`，触发"已下载但引用未就绪 → 标记待重下"机制。详见 [sync 排除 aimDirectory & callout 内部 `<a>` 链接替换](../.req-discuss/feishu/sync-aim-dir-and-callout-link/discuss.md)。
