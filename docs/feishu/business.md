...
- 下载管线解析 `<sub-page-list>` 块为 Markdown 无序列表：块级正则匹配整块，块内逐项解析 `<sub-page doc-id="..." file-type="..." title="..."/>`。命中分支按 `file-type` 分流：`docx` 走 `human_path.md`、`sheet` / `file` 走 `upload_url` 直出（不加 `.md`）。降级路径分三种（均保留原文 + warning）：缺 `doc-id`；`file-type` 非 `docx` / `sheet` / `file`（如 `bitable` / `mindnote`）；`resolveLink` 返回 null。块级决策：当所有子项都是"resolveLink 失败"（格式正确但节点缺失）时丢弃整块；只要存在"命中 / 格式问题保留"中的任一种有效子项，就输出 Markdown UL。`space-id` / `wiki-token` 不参与解析，仅作信息保留。`resolveLink` 回调在"被引方存在但 human_path / upload_url 未就绪"时调用 `incrementNodePriority` 把被引方排到下载队列前面，与 `<cite>` 解析器行为对齐（被引方未就绪的同义信号）；被引方 `obj_token` 不在 DB 时无法 bump（UPDATE 影响 0 行），需先跑 `sync` 把被引节点写进索引。
- 下载管线解析 `<cite>` 引用块为 Markdown 链接：仅处理 `type="doc"` + `file-type="wiki"` 的标准组合，其余组合（`type` 非 `doc`、`file-type` 非 `wiki`）以及缺 `doc-id`、`resolveLink` 返回 null 都保留原文 + warning，提示作者检查标签或重跑 sync。`resolveLink` 回调在"被引方存在但 human_path / upload_url 未就绪"时调用 `incrementNodePriority` 把被引方排到下载队列前面，与 `<sub-page>` 解析器行为对齐。
